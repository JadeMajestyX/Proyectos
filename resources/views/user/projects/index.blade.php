            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="divide-y">
                    @forelse ($projects as $project)
                        <a href="{{ route('user.projects.show', $project) }}" class="block py-3 hover:bg-gray-50 rounded">
                            <div class="font-semibold">{{ $project->name }}</div>
                            <div class="text-sm text-gray-600">{{ $project->description }}</div>
                            <div class="text-xs text-gray-500 mt-1">Creado: {{ $project->created_at->diffForHumans() }}</div>
                        </a>
                    @empty
